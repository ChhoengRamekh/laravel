...
@section('content')
    ...
    @foreach ($post as $value)
      ...
      <div class="card-body">
        <h3>{{$value['title']}}</h3>
        <p>{{$value['body']}}</p>
        <p class="text-success"> Created By: {{$value->user->name}}</p>
        <span>Tags: </span>
      @foreach ($value->tags->pluck('name') as $item)
          @if($item == 'WEB Developer')
            <span class="btn btn-sm" style="background:#999; color:white">{{$item}}</span> &nbsp;
          @elseif($item == 'WEB Programing')
            <span class="btn btn-sm btn-primary">{{$item}}</span> &nbsp;
          @elseif($item == 'Network Engineer')
            <span class="btn btn-sm btn-success">{{$item}}</span> &nbsp;
          @elseif($item == 'Network Infrastructure')
            <span class="btn btn-sm btn-info" style="color: white;">{{$item}}</span> &nbsp;
          @endif
      @endforeach
      <br><br>
        @Auth
